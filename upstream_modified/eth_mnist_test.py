# python3 eth_mnist_test.py --model_number 1 --start_image 0 --end_image 3000 --n_neurons 64
import argparse
import os
...
assignments = torch.load(f"models/assignments_{model_number}_{n_neurons}_neurons_{start_image}_to_{end_image}.pth", map_location=torch.device(device))
# print("assignments loaded")
proportions = torch.load(f"models/proportions_{model_number}_{n_neurons}_neurons_{start_image}_to_{end_image}.pth", map_location=torch.device(device))
...
print(f"{MODEL_NAME}, All activity accuracy: %.4f" % (accuracy["all"] / n_test))